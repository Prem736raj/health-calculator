...
class BpExportManager(@ApplicationContext private val context: Context) {
        ...
        try {
            ...
            FileWriter(file).use { writer ->
                ...
                fun writeRow(values: List<String>) {
                    writer.append(values.joinToString(",") { escapeCsv(it) }).append('\n')
                }

                writeRow(listOf(
                    "Date", "Time", "Systolic (mmHg)", "Diastolic (mmHg)", "Pulse (BPM)",
                    "Category", "Risk Level", "Arm", "Position", "Time of Day", "On Medication",
                    "Medication Name", "Pulse Pressure", "MAP", "Note"
                ))
                ...
                readings.forEach { entity ->
                    ...
                    writeRow(listOf(
                        date,
                        time,
                        entity.systolic.toString(),
                        entity.diastolic.toString(),
                        entity.pulse?.toString().orEmpty(),
                        categoryDisplay,
                        riskDisplay,
                        armDisplay,
                        posDisplay,
                        todDisplay,
                        entity.onMedication.toString(),
                        entity.medicationName,
                        entity.pulsePressure.toString(),
                        String.format(java.util.Locale.US, "%.1f", entity.meanArterialPressure),
                        entity.note
                    ))
                }
            }
    ...
    fun createReadingImage(entity: BloodPressureEntity): Uri? {
        try {
            ...
            canvas.drawText("${ExportDisclosurePolicy.APP_NAME} · Generated by", width / 2f, height - 44f, footerPaint)
            footerPaint.textSize = 10f
            canvas.drawText(
                "${ExportDisclosurePolicy.INFORMATIONAL_LABEL} · ${ExportDisclosurePolicy.DIAGNOSIS_NOTICE}",
                width / 2f,
                height - 20f,
                footerPaint
            )
    ...
    fun exportToPdf(
        ...
        try {
            ...
            ExportDisclosurePolicy.shareFooter().split('\n').forEach { line ->
                canvas.drawText(line, MARGIN, yPos, disclaimerPaint)
                yPos += 12f
            }
    ...
    fun shareFile(uri: Uri, mimeType: String, title: String = "Share Report") {
        val intent = Intent(Intent.ACTION_SEND).apply {
            type = mimeType
            putExtra(Intent.EXTRA_STREAM, uri)
            putExtra(Intent.EXTRA_SUBJECT, "${ExportDisclosurePolicy.APP_NAME} report")
            addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
            addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
        }
